...
def get_counts():
    ...
    teaching_negative_count = df1['teaching'].get(-1,0)
    teaching_neutral_count = df1['teaching'].get(0,0)
    teaching_positive_count = df1['teaching'].get(1,0)

    df1 = df.groupby('coursecontentscore').count()[['coursecontent']]
    coursecontent_negative_count = df1['coursecontent'].get(-1,0)
    coursecontent_neutral_count = df1['coursecontent'].get(0,0)
    coursecontent_positive_count = df1['coursecontent'].get(1,0)

    df1 = df.groupby('examinationscore').count()[['examination']]
    examination_negative_count = df1['examination'].get(-1,0)
    examination_neutral_count = df1['examination'].get(0,0)
    examination_positive_count = df1['examination'].get(1,0)

    df1 = df.groupby('labworkscore').count()[['labwork']]
    labwork_negative_count = df1['labwork'].get(-1,0)
    labwork_neutral_count = df1['labwork'].get(0,0)
    labwork_positive_count = df1['labwork'].get(1,0)

    df1 = df.groupby('libraryfacilitiesscore').count()[['libraryfacilities']]
    libraryfacilities_negative_count = df1['libraryfacilities'].get(-1,0)
    libraryfacilities_neutral_count = df1['libraryfacilities'].get(0,0)
    libraryfacilities_positive_count = df1['libraryfacilities'].get(1,0)

    df1 = df.groupby('extracurricularscore').count()[['extracurricular']]
    extracurricular_negative_count = df1['extracurricular'].get(-1,0)
    extracurricular_neutral_count = df1['extracurricular'].get(0,0)
    extracurricular_positive_count = df1['extracurricular'].get(1,0)
